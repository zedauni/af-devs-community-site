  ...
  <main class="container mx-auto px-4 py-12">
    <div class="mx-auto max-w-4xl">
      ...
      <div class="mb-8 text-center">
        <h1 class="text-gradient-p-to-s mb-4 text-4xl font-bold tracking-tight text-gray-800 sm:text-6xl dark:text-white">
          #{{ tagSlug }}
        </h1>
...
const currentTag = computed<Tag | undefined>(() => 
  allTags.value.find(tag => tag.title.toLowerCase() === tagSlug.value.toLowerCase())
);
const otherTags = computed<Tag[]>(() => 
  allTags.value.filter(tag => tag.title.toLowerCase() !== tagSlug.value.toLowerCase())
);
